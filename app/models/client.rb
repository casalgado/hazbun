class Client < ActiveRecord::Base
  ...
  scope :has_appointment, -> {
    joins(:appointments).group("clients.id").merge(Appointment.appointment_in_future)
   }
  ...
  # Este metodo devuelve el nombre completo de una client
  def nombre_completo
  	[self.nombre, self.apellido].join(" ")
  end

  # Estos dos metodos devuelven las open/closed orders que la client tenga. 
  ...
  # para determinar si la client esta activa o no.
  ...
  # Los siguientes tres metodos son para que una diseñadora pueda crear una client sin contraseña.
  # Luego la client puede crear una contraseña para entrar a su cuenta siguiendo el mail de confirmacion.
  ...
  # el siguiente metodo es para determinar si la client tiene una open order. 
