...
TipoDeCita.load
	...
	describe "de_hoy method returns only citas de hoy" do
		before :each do
			...
			@today     = create(:appointment, date: Time.now, 	   	  		 clienta_id: @clienta.id, tipo_de_cita_id: "2")
			@today_2   = create(:appointment, date: Time.now,  	   	  		 clienta_id: @clienta.id, tipo_de_cita_id: "2")
			@tomorrow  = create(:appointment, date: Time.at(@time_t.to_i), clienta_id: @clienta.id, tipo_de_cita_id: "2")
			@yesterday  = build(:appointment, date: Time.at(@time_y.to_i), clienta_id: @clienta.id, tipo_de_cita_id: "2")
		end
